...
- **A verified active analyzer already reports it.** Confirm Roslyn enforcement or current SonarQube results
  before treating a `CA`/`S`-numbered finding as covered. Cite the rule if you happen across the smell; do not
  duplicate a verified result. See

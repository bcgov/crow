...
- Use the shared `PropertyTestSampling` helper from the managed Crow template (install it the same way as
  ...
  runs use the supplied descriptive seed, so failures reproduce exactly. Set `CsCheck_Randomize=true` for an
  exploratory run that uses CsCheck's default random seeding without changing every test.

...
- For CsCheck properties, install the managed `PropertyTestSampling` helper and call
  `SampleProperty(..., seed: "DescriptiveName")`. The managed helper derives a separate
  reproducible CsCheck seed for each normal-run iteration because CsCheck applies its
  `seed` argument only to the first iteration of a multi-iteration sample. Keep normal
  runs deterministic. Set

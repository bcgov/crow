...
- `.apm/skills/crow-dw-ssas-references/` — Shared, selectively loaded reference corpus migrated from the source DW/SSAS project, with routing index, provenance gate, reusable placeholders for environment-specific tool paths, and deterministic corpus validation
...
```text
apm install bcgov/crow#v0.10.0 --global --target claude
apm install bcgov/crow#v0.10.0 --global --target codex
apm install bcgov/crow#v0.10.0 --global --target cursor
```
...
```powershell
apm install .\build\bcgov-crow-0.10.0.zip --global --target claude
apm install .\build\bcgov-crow-0.10.0.zip --global --target codex
apm install .\build\bcgov-crow-0.10.0.zip --global --target copilot
apm install .\build\bcgov-crow-0.10.0.zip --global --target cursor
```

For Copilot CLI, unpack and install the plugin directory:

```powershell
Expand-Archive .\build\bcgov-crow-0.10.0.zip -DestinationPath .\build\copilot
copilot plugin install .\build\copilot\bcgov-crow-0.10.0
```
